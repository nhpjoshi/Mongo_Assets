...
function displayData(data) {
    ...
    const resultsText = data.map((item, index) => {
        if (item) {
            return `${index + 1}.${item}`;
        } else {
...
function getText() {
    ...
    .catch(error => {
        console.error('Error:', error);
        document.getElementById("outputField").value = 'Error occurred while processing request';
    });
}
...
function sendRequest() {
    ...
    .catch(error => {
        console.error('Error:', error);
        document.getElementById("description").value = 'Error occurred while processing request';
    });
}
